/* Interactive viewer — slice 03, Ecuador SRI control area. Top strip switches
   device (desktop / mobile), state (the 9 SRI states + loading / error) and
   design mood. The SRI control panel mounts inside the Platform Shell chrome so
   tenant context, sidebar, mood and assistant never disappear. Persisted in
   localStorage. */
(function () {
  const { useState, useEffect } = React;
  const { Btn, AssistantPanel, StateScreen } = window.UI;
  const { Sidebar, TopBar } = window.Chrome;
  const { DesktopSRI } = window.SRI;
  const { MobileSRIScreen } = window.MobileSRI;
  const D = window.SRI_DATA;

  const STATES = [
    { key: 'ready', label: 'Healthy / listo' },
    { key: 'blocked', label: 'Bloqueado por readiness' },
    { key: 'pending', label: 'Pendiente de envío' },
    { key: 'submitted', label: 'Enviado · esperando SRI' },
    { key: 'authorized', label: 'Autorizada' },
    { key: 'rejected', label: 'Rechazada' },
    { key: 'unsupported', label: 'Documento no soportado' },
    { key: 'fallback', label: 'Puente XML disponible' },
    { key: 'trace', label: 'Historial técnico' },
    { key: 'loading', label: 'Cargando' },
    { key: 'backend-unavailable', label: 'Backend no disponible' }
  ];

  const LS = 'sri-slice-v1';
  ...
  function DesktopMain({ state }) {
    if (state === 'loading') {
      return (
        <div style={{ padding: 24, display: 'grid', gap: 16, maxWidth: 860, margin: '0 auto' }}>
          <div style={{ height: 18, width: 200, borderRadius: 6, background: 'var(--surface)', border: '1px solid var(--border)' }} />
          <div style={{ height: 78, borderRadius: 'var(--radius-md)', background: 'var(--surface)', border: '1px solid var(--border)' }} />
          <div style={{ height: 430, borderRadius: 'var(--radius-md)', background: 'var(--surface)', border: '1px solid var(--border)' }} />
        </div>
      );
    }
    ...
    return <DesktopSRI d={D} stateKey={state} />;
  }

  function App() {
    const init = load();
    const [device, setDevice] = useState(init.device || 'desktop');
    const [state, setState] = useState(init.state || 'submitted');
      ...
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#0f1115' }}>
        <div style={{ flex: 'none', display: 'flex', alignItems: 'center', gap: 14, padding: '10px 16px', background: '#15181e', borderBottom: '1px solid #262b34', color: '#e6e9ef', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700, fontSize: 13, letterSpacing: '-0.01em', whiteSpace: 'nowrap', flex: 'none' }}>
            ...
            SRI control area
            <span style={{ fontSize: 11, fontWeight: 500, color: '#8b93a3', marginLeft: 2 }}>· Slice 03</span>
          </div>
          <div style={{ flex: 1 }} />
          ...
          {device === 'desktop' ? (
            ...
            <div style={{ height: '100%', display: 'grid', placeItems: 'center', padding: 20, overflow: 'auto' }}>
              <div data-mood={mood} className="ds-app" style={{ width: 390, height: 788, maxHeight: '100%', borderRadius: 30, overflow: 'hidden', border: '10px solid #05070b', boxShadow: '0 24px 70px rgba(0,0,0,0.5)', position: 'relative', background: 'var(--app-bg)' }}>
                {state === 'loading' ? (
                  <div style={{ padding: 14, display: 'grid', gap: 12 }}>{[0, 1, 2, 3].map((i) => <div key={i} style={{ height: i === 0 ? 70 : 58, borderRadius: 'var(--radius-md)', background: 'var(--surface-sunken)', border: '1px solid var(--border)', opacity: 1 - i * 0.14 }} />)}</div>
                  ...
                  <MobileSRIScreen key={state} d={D} stateKey={state} mood={mood} onMood={setMood} />
                )}
              </div>
            </div>
          )}

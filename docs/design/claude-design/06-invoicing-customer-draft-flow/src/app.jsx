/* Interactive viewer — slice 06, Invoicing Customer + Draft Invoice Flow. Top
   strip switches device (desktop / mobile), state (13 explicit states) and
   design mood. The flow mounts inside the real Platform Shell chrome so tenant
   context, sidebar, mood and assistant never disappear. Selection persists in
   localStorage. */
(function () {
  ...
  var DesktopFlow = window.Flow.DesktopFlow;
  var MobileFlowScreen = window.MobileFlow.MobileFlowScreen;
  var D = window.FLOW_DATA;

  var STATES = [
    { key: 'loading', label: '1 · Cargando' },
    { key: 'no_customers', label: '2 · Sin compradores' },
    { key: 'customers_available', label: '3 · Compradores disponibles' },
    { key: 'buyer_selected', label: '4 · Comprador seleccionado' },
    { key: 'buyer_form_partial', label: '5 · Formulario incompleto' },
    { key: 'customer_creating', label: '6 · Creando comprador' },
    { key: 'customer_failed', label: '7 · Error al crear comprador' },
    { key: 'invoice_disabled_no_buyer', label: '8 · Borrador sin comprador' },
    { key: 'invoice_ready', label: '9 · Borrador listo' },
    { key: 'invoice_creating', label: '10 · Creando factura' },
    { key: 'invoice_failed', label: '11 · Error al crear factura' },
    { key: 'invoice_created', label: '12 · Borrador creado' },
    { key: 'permission_limited', label: '13 · Permiso limitado' },
    { key: 'backend_unavailable', label: '14 · Backend no disponible' }
  ];

  var LS = 'customer-draft-flow-slice-v1';
  ...
  function Skeleton() {
    return React.createElement('div', { style: { padding: 'var(--gutter)', maxWidth: 1180, margin: '0 auto', display: 'grid', gap: 16 } },
      React.createElement('div', { style: { height: 26, width: 300, borderRadius: 6, background: 'var(--surface)', border: '1px solid var(--border)' } }),
      React.createElement('div', { style: { height: 74, borderRadius: 'var(--radius-md)', background: 'var(--surface)', border: '1px solid var(--border)' } }),
      React.createElement('div', { style: { display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 320px', gap: 16, alignItems: 'start' } },
        React.createElement('div', { style: { display: 'grid', gap: 16 } },
          [0, 1, 2].map(function (i) { return React.createElement('div', { key: i, style: { height: i === 0 ? 220 : 120, borderRadius: 'var(--radius-md)', background: 'var(--surface)', border: '1px solid var(--border)', opacity: 1 - i * 0.18 } }); })),
        React.createElement('div', { style: { height: 220, borderRadius: 'var(--radius-md)', background: 'var(--surface)', border: '1px solid var(--border)' } })));
  }
  ...
  function DesktopMain(props) {
    ...
    return React.createElement(DesktopFlow, { s: D.scenarios[state] });
  }
  ...
  function App() {
    ...
    var stv = useState(init.state || 'customers_available'); var state = stv[0], setState = stv[1];
    ...
    var topStrip = React.createElement('div', { style: { flex: 'none', display: 'flex', alignItems: 'center', gap: 14, padding: '10px 16px', background: '#15181e', borderBottom: '1px solid #262b34', color: '#e6e9ef', flexWrap: 'wrap' } },
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700, fontSize: 13, letterSpacing: '-0.01em', whiteSpace: 'nowrap', flex: 'none' } },
        ...
        'Customer + Draft flow',
        React.createElement('span', { style: { fontSize: 11, fontWeight: 500, color: '#8b93a3', marginLeft: 2 } }, '· Slice 06')),
    ...
    if (device === 'desktop') {
      ...
      if (state === 'loading') {
        ...
        phoneInner = React.createElement(MobileFlowScreen, { key: state, s: D.scenarios[state], mood: mood, onMood: setMood });
      }

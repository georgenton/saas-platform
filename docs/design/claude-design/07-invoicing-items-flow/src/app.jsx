/* Interactive viewer — slice 07, Invoicing Items Flow. Top strip switches device
   (desktop / mobile), state (11 explicit states) and design mood. The panel
   mounts inside the real Platform Shell chrome. Selection persists in
   localStorage. */
(function () {
  ...
  var DesktopItems = window.Items.DesktopItems;
  var MobileItemsScreen = window.MobileItems.MobileItemsScreen;
  var D = window.ITEMS_DATA;

  var STATES = [
    { key: 'loading', label: '1 · Cargando' },
    { key: 'draft_ready', label: '2 · Draft listo para items' },
    { key: 'no_items', label: '3 · Sin items' },
    { key: 'with_items', label: '4 · Con items' },
    { key: 'form_partial', label: '5 · Formulario incompleto' },
    { key: 'creating_item', label: '6 · Creando item' },
    { key: 'create_failed', label: '7 · Error al crear item' },
    { key: 'no_tax_rates', label: '8 · Sin impuestos activos' },
    { key: 'non_draft_constrained', label: '9 · No-draft (constrained)' },
    { key: 'permission_limited', label: '10 · Permiso limitado' },
    { key: 'backend_unavailable', label: '11 · Backend no disponible' }
  ];

  var LS = 'invoicing-items-flow-slice-v1';
  ...
  function Skeleton() {
    return React.createElement('div', { style: { padding: 'var(--gutter)', maxWidth: 1180, margin: '0 auto', display: 'grid', gap: 16 } },
      React.createElement('div', { style: { height: 26, width: 280, borderRadius: 6, background: 'var(--surface)', border: '1px solid var(--border)' } }),
      React.createElement('div', { style: { height: 96, borderRadius: 'var(--radius-md)', background: 'var(--surface)', border: '1px solid var(--border)' } }),
      React.createElement('div', { style: { display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 320px', gap: 16, alignItems: 'start' } },
        React.createElement('div', { style: { display: 'grid', gap: 16 } },
          [0, 1].map(function (i) { return React.createElement('div', { key: i, style: { height: i === 0 ? 200 : 260, borderRadius: 'var(--radius-md)', background: 'var(--surface)', border: '1px solid var(--border)', opacity: 1 - i * 0.15 } }); })),
  ...
  function DesktopMain(props) {
    ...
    return React.createElement(DesktopItems, { s: D.scenarios[state] });
  }
  ...
  function App() {
    ...
    var stv = useState(init.state || 'with_items'); var state = stv[0], setState = stv[1];
    ...
    var topStrip = React.createElement('div', { style: { flex: 'none', display: 'flex', alignItems: 'center', gap: 14, padding: '10px 16px', background: '#15181e', borderBottom: '1px solid #262b34', color: '#e6e9ef', flexWrap: 'wrap' } },
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700, fontSize: 13, letterSpacing: '-0.01em', whiteSpace: 'nowrap', flex: 'none' } },
        ...
        'Invoicing items flow',
        React.createElement('span', { style: { fontSize: 11, fontWeight: 500, color: '#8b93a3', marginLeft: 2 } }, '· Slice 07')),
    ...
    if (device === 'desktop') {
      ...
      if (state === 'loading') {
        ...
        phoneInner = React.createElement(MobileItemsScreen, { key: state, s: D.scenarios[state], mood: mood, onMood: setMood });
      }

/* Platform Shell chrome reused for the Invoicing slice — sidebar, desktop top
   bar, mobile top bar, bottom tabs and bottom sheet. Lifted from slice 00 so
   the shell, mood plumbing and tenant context never disappear; this slice only
   designs the <main> content. window.Chrome. */
(function () {
  const { useState } = React;
  const Icon = window.Icon;
  const { Btn, Pill, Avatar, Brand, NavRow, MoodMenu } = window.UI;
  const GROUPS = ['Core', 'Finance', 'Commerce', 'Clinics', 'Platform'];

  /* ----------------------------------------------------------- Sidebar */
  function Sidebar({ products, activeKey, user }) {
    return (
      <aside style={{
          ...
          {GROUPS.map((g) => {
            ...
            return (
              <div key={g} style={{ marginBottom: 14 }}>
                ...
                {items.map((p) => <NavRow key={p.key} p={p} active={p.key === activeKey} onClick={() => {}} />)}
              </div>
            );
          })}
        </nav>
        <div style={{ padding: 12, borderTop: '1px solid var(--divider)', display: 'grid', gap: 10 }}>
          <Btn variant="secondary" full leading="marketplace">Add products</Btn>
  ...
  /* ------------------------------------------------------------ TopBar */
  function TopBar({ tenant, memberships, mood, moods, onMood, onAssistant }) {
    ...
    return (
      <header style={{ height: 60, flex: 'none', background: 'var(--topbar-bg)', borderBottom: '1px solid var(--topbar-border)', display: 'flex', alignItems: 'center', gap: 14, padding: '0 18px', position: 'relative', zIndex: 5 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--text-muted)', fontSize: 'var(--text-sm)' }}>
          ...
          <span style={{ fontWeight: 600, color: 'var(--text-strong)' }}>Invoicing</span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--surface-sunken)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '0 10px', height: 36, width: 180, color: 'var(--text-subtle)' }}>
          ...
          <span style={{ fontSize: 'var(--text-sm)' }}>Buscar factura…</span>
        </div>
  ...
  /* ------------------------------------------------------- Mobile frame */
  const TABS = [
    { key: 'overview', name: 'Resumen', icon: 'home' },
    { key: 'queue', name: 'Facturas', icon: 'invoicing', badge: 3 },
    { key: 'readiness', name: 'SRI', icon: 'shield' },
  ...
  function Sheet({ title, children, onClose }) {
    return (
      <div onClick={onClose} style={{ position: 'absolute', inset: 0, zIndex: 30, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
        ...
        <div onClick={(e) => e.stopPropagation()} style={{ position: 'relative', background: 'var(--surface-raised)', borderTopLeftRadius: 16, borderTopRightRadius: 16, padding: 16, maxHeight: '82%', overflowY: 'auto', boxShadow: 'var(--shadow-lg)' }}>
  ...
  window.Chrome = { Sidebar, TopBar, MobileTopBar, BottomTabs, Sheet, TABS };
})();

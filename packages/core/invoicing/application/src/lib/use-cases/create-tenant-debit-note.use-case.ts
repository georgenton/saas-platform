...
import { InvalidDebitNoteSourceInvoiceError } from '../errors/invalid-debit-note-source-invoice.error';
...
export interface CreateTenantDebitNoteInput {
  tenantSlug: string;
...
export interface CreateTenantDebitNoteResult {
  debitNote: Invoice;
  sourceInvoice: Invoice;
  initialItem: InvoiceItem;
}

export class CreateTenantDebitNoteUseCase {
  constructor(
  ...
  async execute(
    input: CreateTenantDebitNoteInput,
  ): Promise<CreateTenantDebitNoteResult> {
    ...
    if ((sourceInvoice.documentCode ?? '01') !== '01') {
      throw new InvalidDebitNoteSourceInvoiceError(
        input.tenantSlug,
        input.sourceInvoiceId,
      );
    }
    ...
    const debitNote = Invoice.create({
    ...
    await this.invoiceRepository.save(debitNote);
